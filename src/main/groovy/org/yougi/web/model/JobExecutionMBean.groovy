...
class JobExecutionMBean {
  ...
  void load() {
    if(EntitySupport.isIdValid(id)) {
      jobExecution = jobExecutionBean.find(id)
    } else {
  ...
  String remove() {
    jobExecution = jobExecutionBean.find(jobExecution.id)
    JobScheduler jobScheduler = jobExecution.getJobScheduler()
    jobExecutionBean.remove(jobExecution.id)
    'job_scheduler?faces-redirect=true&id='+ jobScheduler.id
  }
}
