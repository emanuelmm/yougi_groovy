...
public final class Base64Encoder {
     ...
     * @return A character array with the Base64 encoded data.
     */
    public static char[] encode(byte[] inp, int iLen) {
        ...
        while (ip < iLen) {
            int i0 = inp[ip++] & 0xff;
            int i1 = ip < iLen ? inp[ip++] & 0xff : 0;
            int i2 = ip < iLen ? inp[ip++] & 0xff : 0;
            int o0 = i0 >>> 2;
            int o1 = ((i0 & 3) << 4) | (i1 >>> 4);
     ...
     * data.
     */
    public static byte[] decode(char[] inp) {
        int iLen = inp.length;
        if (iLen % 4 != 0) {
            throw new IllegalArgumentException("Length of Base64 encoded input string is not a multiple of 4.");
        }
        while (iLen > 0 && inp[iLen - 1] == '=') {
            iLen--;
        }
        int oLen = (iLen * 3) / 4;
        ...
        while (ip < iLen) {
            int i0 = inp[ip++];
            int i1 = inp[ip++];
            int i2 = ip < iLen ? inp[ip++] : 'A';
            int i3 = ip < iLen ? inp[ip++] : 'A';
