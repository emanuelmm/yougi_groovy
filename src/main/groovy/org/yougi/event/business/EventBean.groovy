 ...
 */
@Stateless
class EventBean extends AbstractBean<Event> {
  ...
  List<Event> findParentEvents() {
    List<Event> events =  em.createQuery("select e from Event e where e.parent is null order by e.endDate desc", Event)
      .getResultList()
    loadVenues(events)
  }

  List<Event> findEvents(Event parent) {
    List<Event> events = em.createQuery("select e from Event e where e.parent.id = :parent order by e.startDate asc", Event)
  ...
  void sendConfirmationEventAttendance(UserAccount userAccount, Event event, String dateFormat, String timeFormat, String timezone) {
