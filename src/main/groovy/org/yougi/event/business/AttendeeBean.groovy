 ...
 */
@Stateless
class AttendeeBean extends AbstractBean<Attendee> {
  ...
  Attendee find(Event event, UserAccount person) {
    Attendee attendee = null
    List<Attendee> attendees = em.createQuery('select a from Attendee a where a.userAccount = :person and a.event = :event', Attendee)
  ...
  Long findNumberPeopleAttending(Event event) {
    (Long) em.createQuery('select count(a) from Attendee a where a.event = :event').setParameter('event', event).getSingleResult()
  }

  Long findNumberPeopleAttended(Event event) {
    (Long) em.createQuery('select count(a) from Attendee a where a.event = :event and a.attended = :attended').setParameter('event', event).setParameter('attended', true).getSingleResult()
  }

  Boolean isAttending(Event event, UserAccount person) {
    try {
      Attendee attendee = em.createQuery('select a from Attendee a where a.userAccount = :person and a.event = :event', Attendee)
        .setParameter('person', person)
        .setParameter('event', event)
        .getSingleResult()
   ...
   */
  List<Attendee> findAttendees(Event event) {
    em.createQuery('select a from Attendee a where a.event = :event order by a.userAccount.firstName asc', Attendee).setParameter('event', event).getResultList()
  }
   ...
   * @return a list of found attendees.
   */
  List<Attendee> findAllAttendees(Event event) {
  ...
  List<Attendee> findAttendeesWhoAttended(Event event) {
    em.createQuery('select a from Attendee a where a.event = :event and a.attended = :attended order by a.userAccount.firstName asc', Attendee).setParameter('event', event).setParameter('attended', true).getResultList()
  }

  /**
   * Returns a list of events in which the presence of the user was confirmed.
   */
  List<Event> findAttendeedEvents(UserAccount userAccount) {
    return em.createQuery('select a.event from Attendee a where a.userAccount = :userAccount and a.attended = :attended order by a.event.startDate desc', Event)
   ...
   */
  void confirmMembersAttendance(Event event, Attendee[] confirmedAttendees) {
   ...
   */
  Boolean verifyCertificateAuthenticity(Certificate certificate) {
    try {
      Attendee attendee = em.createQuery('select a from Attendee a where a.certificateCode = :certificateCode and a.certificateFullname = :certificateFullname and a.certificateEvent = :certificateEvent and a.certificateVenue = :certificateVenue', Attendee)
